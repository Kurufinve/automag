...
# filepath = os.path('espins/*_mc.dat')
filename = os.popen('ls espins/*_mc.dat').read().rstrip()
print(f"ESPinS data will be read from {filename}")

results = np.loadtxt(filename)
temperature = results[:, 0]
magnetization = results[:, 1]/
energy_ave = results[:, 2]
c_m = results[:, 3]
sus = results[:, 4]
u_e = results[:, 5]
u_m = results[:, 6]
...
plt.legend()
# plt.show()
plt.savefig('magnetization_espins.png')
